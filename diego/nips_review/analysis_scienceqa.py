...
dataset_path = "/home/diego/Documents/area_science/ricerca/open/geometric_lens/repo/diego/science_qa/category_partition"
dataset = load_from_disk(f"{dataset_path}/test")


base_path = "/home/diego/Documents/area_science/ricerca/open/geometric_lens/repo/results/scienceqa"
subset = dataset
sub_to_int = {sub: i for i, sub in enumerate(np.unique(subset["category"]))}
int_to_sub = {i: sub for sub, i in sub_to_int.items()}
int_labels = np.array([sub_to_int[sub] for sub in subset["category"]])
...
for sub, value in strat_overlaps_5shot.items():
    ...
    delta = ov_gt - ov_gt0
    if ov_gt > 0.82 and n_points > 70 and ov_gt - ov_gt0 > 0.2:
...
for topic in subj_to_select:
    ind = np.nonzero(np.array(dataset["category"]) == topic)[0]
    assert len(ind) > 0, topic
    indices.extend(list(ind[:1000]))

indices = np.arange(len(dataset["category"]))
# len(indices)


sub_to_int = {sub: i for i, sub in enumerate(np.unique(dataset["category"]))}
int_to_sub = {i: sub for sub, i in sub_to_int.items()}
int_labels = np.array([sub_to_int[sub] for sub in dataset["category"]])
...
assignment = d.compute_clustering_ADP(Z=5, halo=False)
d.N_clusters
