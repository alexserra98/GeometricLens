...
X = torch.load(f"{model_path}/l7_target.pt")
...
fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(13, 8))  # create figure & 1 axis
...
ax.set_xticks([])
plt.tight_layout()
# plt.savefig(f"{results_folder}/llama3-70b-5shot_app.pdf")
...
#     [0, 3, 5, 8],
# )
xticks = [0.07, 0.23, 0.38, 0.51, 0.64, 0.78, 0.9, 0.97]
xticklabels = [
    "enigineering",
    "chemistry",
    "conceptual\nphysics",
...
for link in range(nlinks):
    for i in range(3):
        x0, y0 = xcoords[link, i], ycoords[link, i]
        x1, y1 = xcoords[link, i + 1], ycoords[link, i + 1]
        ax.plot([x0, x1], [y0, y1], color="C0", lw=1.5)  # color = cl[c_nums[link]]
...
gs.tight_layout(fig)
plt.savefig("final_dendogram_llama3-70b_5shot.png", dpi=400)
