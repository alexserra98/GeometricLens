...
class LabelClustering(HiddenStatesMetrics):
    ...
    def main(self, label) -> pd.DataFrame:
        ...
        for z in tqdm.tqdm(iter_list, desc = "Computing overlap"):
            for n, query_dict in tqdm.tqdm(enumerate(self.queries), desc="Processing queries"):
                ...
                row = [query_dict["model"],query_dict["method"],query_dict["train_instances"],z]
                ...
                try:
                    ...
                    module_logger.error(f"Error computing clustering for query {query_dict}: {e}")
                    raise e
                ...
                if n % 3 == 0:
                    ...
                    df_temp.to_pickle(check_point_dir / f"checkpoint_{self.label}_cluster.pkl")
    ...
    def process_layer(self, layer, hidden_states, label, z) -> dict:
        ...
        base_unique, base_idx, base_inverse = np.unique(
            hidden_states[:,layer,:], axis=0, return_index=True, return_inverse=True
        )
        indices = np.sort(base_idx)
        base_repr = base_unique[indices]
