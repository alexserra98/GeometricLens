...
class HiddenStates:
    ...
    def intrinsic_dim(self) -> pd.DataFrame:
        intrinsic_dim = IntrinsicDimension(
            df=self.df, tensor_storage=self.tensor_storage, variations=self.variations
        )
        ...
        for dataset in tqdm.tqdm(self.df["dataset"].unique().tolist()):
            for model in self.df["model_name"].unique().tolist():
                ...
                for train_instances in self.df["train_instances"].unique().tolist():
                    ...
                    _, _, hidden_states_df = hidden_states_collapse(
                        self.df, self.tensor_storage, query
                    )
                    rows.append(
                        [
                            dataset,
                            ...
                            hidden_states_df.apply(
                                lambda r: quasi_exact_match(
                                    r["std_pred"], r["letter_gold"]
                                ),
                                axis=1,
                            ).mean(),
                            hidden_states_df.apply(
                                lambda r: exact_match(
                                    r["only_ref_pred"], r["letter_gold"]
                                ),
                                axis=1,
                            ).mean(),
                        ]
    ...
    def label_clustering(self, label: str, **kwargs) -> pd.DataFrame:
        ...
        if variation:
            label_clustering = LabelClustering(
                df=self.df, tensor_storage=self.tensor_storage, variation=variation
            )
        else:
            label_clustering = LabelClustering(
                df=self.df, tensor_storage=self.tensor_storage
            )
    ...
    def cka(self) -> pd.DataFrame:
        ck_align = CenteredKernelAlignement(
            df=self.df, tensor_storage=self.tensor_storage, variations=self.variations
        )
