...
class LinearProbe(HiddenStatesMetrics):
    def main(self, label) -> pd.DataFrame:
        ...
        for query_dict,n in tqdm.tqdm(enumerate(self.queries), desc="Processing queries:"):
            ...
            try:
                accuracies = self.parallel_compute(train_folds, 
                                                test_folds, 
                                                class_weight_folds, 
                                                n_folds)
            ...
            row = [query_dict["model"], query_dict["method"], query_dict["shot"], accuracies]
            rows.append(row)
    ...
    def parallel_compute(self, 
        ...
        with Parallel(n_jobs=_NUM_PROC) as parallel:
            results = parallel(
                ...
                for num_layer in tqdm.tqdm(range(train_folds[0][0].shape[1]), desc="Processing layers:")
            )
    ...
    def process_layer(self, 
        ...
        for i in range(n_folds):
            ...
            model = LogisticRegression(max_iter=4000, class_weight=class_weights_folds[i])
